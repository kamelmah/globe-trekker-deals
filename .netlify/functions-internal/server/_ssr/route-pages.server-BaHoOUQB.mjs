import { c as formatMonthLong, r as formatDateMedium } from "./dates-DNk5GF2y.mjs";
...
import { c as findWhitelistedRoute, f as routesFrom, l as frenchName, n as COUNTRY_NAMES_FR, r as DESTINATIONS, t as AIRPORT_NAMES_FR } from "./route-whitelist-w8ea1sr9.mjs";
import { getCityIndex } from "./geo.server-CRol7P84.mjs";
import { n as slugify, t as routeSlug } from "./slug-BO_x1oHK.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/route-pages.server-BaHoOUQB.js
/**
...
function getSlugIndex() {
	if (!slugIndexPromise) slugIndexPromise = buildSlugIndex().catch((error) => {
...
*/
async function readWorldCache(origin) {
	try {
		const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
		const { data } = await supabaseAdmin.from("price_cache").select("cache_key,payload,expires_at").like("cache_key", `world-destinations:${origin}:%`).order("expires_at", { ascending: false }).limit(4);
		const entries = [];
		let allStale = (data?.length ?? 0) > 0;
		let freshestKey = null;
		for (const row of data ?? []) {
			if (Date.parse(row.expires_at) >= Date.now()) allStale = false;
			freshestKey ??= row.cache_key;
			const prices = row.payload?.prices;
			if (!Array.isArray(prices)) continue;
			for (const entry of prices) if (entry?.destination && entry?.city && Number.isFinite(Number(entry.priceEur))) entries.push(entry);
		}
		if (allStale && freshestKey) revalidateWorldCache(origin, freshestKey);
		return entries;
	} catch (error) {
...
async function revalidateWorldCache(origin, cacheKey) {
	if (revalidating.has(cacheKey)) return;
	revalidating.add(cacheKey);
	try {
		const month = cacheKey.split(":")[2];
		const { fetchCheapestDestinations } = await import("./travelpayouts.server-Dj_hGfma.mjs");
...
async function resolveRouteSlug(slug) {
	...
	for (let cut = 1; cut < parts.length; cut += 1) {
		...
		const matchesDestination = (entry) => slugify(entry.city) === destSlug || slugify(frenchName(entry.destination) ?? "") === destSlug;
		const cached = (await readWorldCache(origin.code)).filter((e) => matchesDestination(e) && e.destination !== origin.code).sort((a, b) => Number(a.priceEur) - Number(b.priceEur))[0];
		if (cached) return {
			origin,
			destination: {
		...
		if (destination && destination.code !== origin.code) return {
			origin,
			destination,
			cached: null
		};
...
async function readObservedPrice(origin, destination, cached) {
	let best = cached ? {
	...
	try {
		const { supabaseAdmin } = await import("./client.server-KzwUIAkW.mjs");
		const { data: history } = await supabaseAdmin.from("price_history").select("lowest_price,updated_at").eq("origin", origin).eq("destination", destination).order("lowest_price", { ascending: true }).limit(1);
		const historyLow = history?.[0] ? Math.round(Number(history[0].lowest_price)) : null;
		if (historyLow && historyLow > 0 && (!best || historyLow < best.priceEur)) best = {
			priceEur: historyLow,
...
function durationLabel(km) {
	const hours = km / 800 + .5;
...
*/
async function buildDynamicRoutePage(slug) {
	...
	const nonstop = findWhitelistedRoute(routeSlug(origin.city, destination.city))?.nonstop ?? true;
	const distanceSentence = nonstop ? `La distance entre ${origin.city} et ${destination.city} (${destination.country}) est d'environ ${km.toLocaleString("fr-FR")} km, soit ${durationLabel(km)} pour un vol direct. Les itinéraires avec une ou deux escales sont souvent moins chers mais rallongent sensiblement le trajet : nos filtres vous permettent d'exclure les escales trop longues en un clic.` : `Aucune compagnie n'assure ${origin.city} — ${destination.city} sans escale : tous les itinéraires passent par une correspondance. La distance à vol d'oiseau est d'environ ${km.toLocaleString("fr-FR")} km, mais comptez sensiblement plus que les ${durationLabel(km)} théoriques, selon la durée de l'escale. Nos filtres permettent d'écarter les correspondances les plus longues.`;
	const metaDescription = priceLabel ? `Prix le plus bas relevé sur ${origin.city} — ${destination.city} (${destination.country}) : ${priceLabel}, taxes incluses, vendeur affiché. Comparez sans frais cachés ni faux compte à rebours.` : `Comparez les vols ${origin.city} — ${destination.city} (${destination.country}) : prix total taxes incluses, vendeur réel identifié et lien direct, sans frais cachés.`;
	const observedDay = frenchDay(observed?.observedAt ?? null);
	const intro = priceLabel ? `Le prix le plus bas que nous avons relevé sur la liaison ${origin.city} — ${destination.city} est de ${priceLabel} taxes incluses${observedMonth ? `, pour un départ en ${observedMonth}` : ""}${observed?.airline ? `, opéré par ${observed.airline}` : ""}${observedDay ? `, relevé le ${observedDay}` : ""}. Ce montant provient de nos relevés de prix réels : il n'est ni arrondi, ni simulé.` : `Aucun relevé de prix n'est encore enregistré sur ${origin.city} — ${destination.city}. Lancez une recherche en direct pour obtenir les tarifs réels du jour, taxes incluses et vendeur identifié.`;
	const sections = [
		{
			heading: `Combien coûte un vol ${origin.city} — ${destination.city} ?`,
			paragraphs: [priceLabel ? `Sur ce trajet, notre plancher observé est de ${priceLabel} taxes comprises. Les prix affichés sur TrouveMonVol sont ceux réellement renvoyés par les vendeurs : vous voyez le montant total dès la liste de résultats, sans supplément découvert au moment du paiement. Chaque offre indique qui vend le billet — la compagnie elle-même ou l'agence précise — et le bouton de réservation ouvre en un clic le lien de réservation de ce vendeur, sans comparateur intermédiaire caché.` : `Les tarifs de cette liaison varient fortement selon la saison, le jour de la semaine et l'anticipation. Nous n'affichons aucun prix estimé : tant qu'aucun relevé n'existe sur ${origin.city} — ${destination.city}, seule une recherche en direct vous donnera un montant, toujours taxes incluses et avec le vendeur réel identifié.`, distanceSentence]
		},
		{
			heading: `Quand réserver pour ${destination.city} ?`,
			paragraphs: [`Comme sur la plupart des liaisons, les meilleures occasions apparaissent en dehors des vacances scolaires et des week-ends de grands départs. Un départ en milieu de semaine, un mardi ou un mercredi, est en général moins cher qu'un vendredi ou un dimanche. Le calendrier des prix intégré au formulaire de recherche affiche, pour chaque jour du mois, le prix le plus bas réellement trouvé : les jours sans donnée restent volontairement vides plutôt que remplis d'une estimation.`, `Si vos dates sont souples, activez l'option « dates flexibles ± 3 jours » : la recherche interroge alors plusieurs journées autour de la date choisie et conserve la meilleure offre. Vous pouvez aussi créer une alerte prix sans créer de compte : nous vous prévenons par e-mail dès que le tarif de ${origin.city} — ${destination.city} baisse.`]
		},
		{
			heading: `Réserver ${origin.city} — ${destination.city} en toute transparence`,
			paragraphs: [`TrouveMonVol ne diffuse aucune publicité tierce, aucun faux compte à rebours et ne met en avant aucune offre payée. Le classement se fait par prix total réel. Chaque résultat affiche l'empreinte carbone estimée du vol ; pour les bagages, nous vous renvoyons vers la page du vendeur plutôt que d'afficher une inclusion que notre source de prix ne garantit pas.`, `Notre rémunération vient d'une commission versée par le vendeur lorsque vous réservez, sans surcoût pour vous et sans influence sur l'ordre des résultats. Le trajet ${origin.city} — ${destination.city} est comparé avec exactement les mêmes règles que tous les autres.`]
		}
	];
	const faq = [
		{
			question: `Quel est le prix le plus bas relevé sur un vol ${origin.city} — ${destination.city} ?`,
			answer: priceLabel ? `Notre plus bas relevé sur ce trajet est de ${priceLabel} taxes incluses${observedMonth ? ` pour un départ en ${observedMonth}` : ""}. Ce prix provient d'un relevé réel et évolue selon les dates : lancez une recherche en direct pour connaître le tarif du jour.` : `Aucun relevé n'est encore enregistré sur cette liaison. Lancez une recherche en direct : les prix affichés proviennent uniquement des offres réelles renvoyées par les vendeurs.`
		},
		{
			question: `Combien de temps dure le vol ${origin.city} — ${destination.city} ?`,
			answer: nonstop ? `La distance est d'environ ${km.toLocaleString("fr-FR")} km, ce qui représente ${durationLabel(km)} sur un vol direct. Avec escale, comptez plusieurs heures supplémentaires selon la correspondance.` : `Cette liaison n'est pas desservie sans escale. La distance est d'environ ${km.toLocaleString("fr-FR")} km, soit ${durationLabel(km)} de vol pur, auxquelles s'ajoute la correspondance — souvent plusieurs heures selon l'itinéraire retenu.`
		},
		{
	...
	return {
		...
		...cached?.destinationAirport ? { observedDestinationAirport: cached.destinationAirport } : {}
	};
}
/**
...
*/
async function listRelatedRoutes(params) {
	const limit = params.limit ?? 12;
	const origin = params.origin.toUpperCase();
	const exclude = params.exclude?.toUpperCase();
	const whitelisted = routesFrom(origin).filter((route) => route.destination !== exclude).map((route) => ({
		destination: route.destination,
		slug: route.slug,
		city: route.destinationCity,
		country: route.country
	}));
	const siblings = whitelisted.length > 0 ? whitelisted : withoutPruned(DESTINATIONS, PRUNED_ROUTE_SLUGS).filter((route) => route.origin.toUpperCase() === origin && route.destination !== exclude).map((route) => ({
		destination: route.destination,
		slug: route.slug,
		city: route.destinationCity,
		country: route.country
	...
	for (const entry of await readWorldCache(origin)) {
		...
		if (current === void 0 || price < current) observed.set(entry.destination, price);
	}
	return siblings.map((route) => ({
		slug: route.slug,
		city: route.city,
		country: route.country,
		priceEur: observed.get(route.destination) ?? null
	})).sort((a, b) => (a.priceEur ?? Infinity) - (b.priceEur ?? Infinity)).slice(0, limit);
}
//#endregion
export { buildDynamicRoutePage, listRelatedRoutes };
